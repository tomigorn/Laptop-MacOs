...
`terminal/SETUP_VERSION` is the single source of truth for the terminal-setup
version. It is shown in the fish greeting — both locally on the Mac and on every
`xxhc` connect (via `XXH_SETUP_VERSION`) — so the version on screen must always
reflect the latest change. (Named `SETUP_VERSION`, not `VERSION` — a bare
`version` variable is reserved in fish.)

**Rule:** whenever you change anything that affects the terminal setup (any file
under `terminal/`), bump `terminal/SETUP_VERSION` in the SAME commit. No terminal
change may land without a matching bump.
...
`SETUP_VERSION` live) and on the next `xxhc` connect — that's the at-a-glance
check that the latest changes are live.
